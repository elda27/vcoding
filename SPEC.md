...
### 7.3 アプリケーションデータディレクトリ

vcoding の作業ファイル（SSH鍵、一時ファイル、ログ等）は、**プロジェクトディレクトリ外**に配置する。
これにより、ユーザーのプロジェクトを汚染せず、成果物のみに集中できる環境を提供する。

#### 7.3.1 ベースディレクトリ

OS に応じたアプリケーションデータディレクトリを使用する。

| OS      | ベースディレクトリ                                              |
| ------- | --------------------------------------------------------------- |
| Linux   | `~/.vcoding`                                                    |
| macOS   | `~/.vcoding`                                                    |
| Windows | `%APPDATA%\vcoding` (`C:\Users\<user>\AppData\Roaming\vcoding`) |

#### 7.3.2 ユースケースと編集ターゲット

vcoding は以下の2つのユースケースをサポートする。

##### ユースケース1: 単一ファイル編集・多数ファイルへの適用検証

- 1つのソースファイルを編集し、その変更が多数のファイルに適切に適用可能か検証する
- 例: 共通ライブラリの修正を複数プロジェクトでテスト

```python
# 単一ファイルをターゲットとして指定
workspace = create_workspace(target="/home/user/lib/utils.py")
```

##### ユースケース2: フォルダ全体の編集

- 特定のフォルダをワークスペースとして、そのワークスペース内のファイルを大量に編集する
- 例: プロジェクト全体のリファクタリング

```python
# フォルダをターゲットとして指定
workspace = create_workspace(target="/home/user/my-project")
```

#### 7.3.3 ワークスペース識別

**編集ターゲット**（ファイルまたはフォルダのパス）を **SHA-256 ハッシュ化**して識別子とする。

- ターゲットパスの正規化（絶対パス化、末尾スラッシュ統一）後にハッシュ化
- ハッシュの最初の2文字を親ディレクトリとする（ファイルシステムの効率化）
- 完全なハッシュ値を子ディレクトリとする

```
例: /home/user/my-project をターゲットとした場合（ハッシュが abc123def456...）

~/.vcoding/
└── workspaces/
    └── ab/                          # ハッシュの最初2文字
        └── abc123def456.../         # 完全なハッシュ値
            ├── config.json          # ワークスペース設定
            ├── metadata.json        # ターゲットパス等のメタ情報
            ├── keys/                # SSH鍵
            ├── temp/                # 一時ファイル（コンテナにマウント）
            └── logs/                # ログファイル
```

#### 7.3.4 メタデータファイル

各ワークスペースディレクトリに `metadata.json` を配置し、ターゲットとの対応を記録する。
...
```json
{
  ...
  "last_accessed": "2026-01-17T15:30:00Z"
}
```

| フィールド    | 説明                                           |
| ------------- | ---------------------------------------------- |
| `target_path` | 編集ターゲットのパス（ファイルまたはフォルダ） |
| `target_type` | `"file"` または `"directory"`                  |

#### 7.3.5 同期履歴の管理

`auto_remove=True` によりコンテナ停止時にコンテナが削除されるため、コンテナ内にコピーしたファイルも失われる。
次回 `start_workspace()` 時に自動的に再コピーするため、同期履歴を `metadata.json` に記録する。
...
```json
{
  ...
  "last_accessed": "2026-01-17T15:30:00Z",
  "synced_files": [
    {"source": "/home/user/my-project/src", "destination": "/workspace/src"},
    {"source": "/home/user/my-project/pyproject.toml", "destination": "/workspace/pyproject.toml"}
  ]
}
```
...
1. **記録**: `sync_to_container()` 実行時に同期したパスを `synced_files` に追加
2. **再コピー**: `start_workspace()` 時に `synced_files` に基づいて自動的に再コピー
3. **存在チェック**: 再コピー時にソースファイルが存在しない場合はスキップし、WARNING ログを出力

##### 同期履歴のクリーンアップ

存在しないソースファイルへの参照が蓄積すると、起動時に WARNING が出続けて煩わしい。
`prune_synced_files()` 関数により、存在しないファイルへの参照を同期履歴から削除できる。

```python
# 存在しないソースファイルへの参照を同期履歴から削除
workspace.prune_synced_files()
```

#### 7.3.6 成果物の同期

- コンテナ内での作業結果は、明示的な同期操作でのみプロジェクトディレクトリに反映
- `sync_from_container()` により、指定された成果物のみをプロジェクトパスにコピー
- 中間ファイル・一時ファイルはワークスペースディレクトリに留まる

#### 7.3.6 ディレクトリ構造全体

```
<app_data_dir>/vcoding/
├── config.json              # グローバル設定（オプション）
└── workspaces/
    ├── ab/
    │   └── abc123.../
    │       ├── config.json
...
- `destroy_workspace()` 時にワークスペースディレクトリごと削除
- 孤立したワークスペース（存在しないプロジェクトパスを参照）を検出・削除するユーティリティを提供
